...
export default function TaskEditModalBody({ task, closeModal, isRunning, trackedSeconds, taskActions }) {
    ...
    const isTrackedMoreThanPlanned = formatedTrackedTime > time
        ...
        <div id={`taskEditModalBody_${id}`} className={`flex gap-2`}>
            ...
            <div className='flex flex-col gap-2 w-full'>
                <div className='flex flex-col pl-8 gap-2 w-full'>
                    ...
                    {(formatedTrackedTime || (isRunning && trackedSeconds)) > 0 &&
                        <div className='my-2'>
                            <Bar percent={formatedTrackedTime / time * 100} overflowEffect={true} />
                        </div>
                    }
                </div>
                ...
                <div className='flex gap-2 items-start'>
                    ...
                    <div id={`timeBox_${id}`} className='flex space-x-1'>
                        ...
                        <div>
                            {isRunning
                                ? <TimeFlagTracking tracked={trackedTime + (isRunning ? trackedSeconds : 0)}
                                    onClick={stopTracking} />
                                : <Input
                                    placeholder="0"
                                    type="number"
                                    width='w-16'
                                    value={secondsToMinutes(trackedTime)}
