...
export default function TaskItem({ task, onToggle, onDelete }) {
    ...
    return (
        <div className='group task-item flex justify-between' onClick={() => onToggle(id)}>
            ...
            >{done ? '✦' : ''}</span>
            <span className={(done ? 'line-through text-text-muted' : 'text-text-primary') + ' select-none'}>
                {label}
            </span>
            <div className='flex gap-2 items-center'>
                <span className='w-12 bg-border-soft text-text-primary px-0.5 rounded-sm text-sm select-none'>{time} min</span>
