export default function EmojiMiniCard({ id, title, subtitle, emoji, onClick, active = false, otherActive, success }) {

   const borderColor = active && success ? 'border-success/50' : (otherActive ? 'border-white' : 'border-yellow-600/50')
   const bgColor = active && success ? 'bg-gradient-success' : (active && !success ? 'bg-gradient-main' : 'bg-bg-base gradientHover')
