...
- ✅ 2026-09-16: "Clear day" — right-click the active list to send every non-running Active task back to Next Up (`moveAllTasksToNextUp`, new `ContextMenu.jsx` mirroring `Modal.jsx`'s portal/escape pattern). Non-destructive by design, for mental clarity on an overwhelming day rather than losing tasks; the currently-tracked task is excluded so it never interrupts what's running.
...
- 🔷 Quick-add icon (+) on Next Up items — one click moves the task straight to Active, no modal or drag needed.
...
### 🟡 Phase 13 — Projects MVP
A first, deliberately simpler version of "projects" than the original fuller design idea (that one's still sketched under Phase 5 below; its old unused code moved to `design/old-projectView/` as reference material, not deleted).
- ✅ 2026-09-16: `/project` page — grid of projects (single project just fills the row via CSS `auto-fit`, no branching needed), create via a "+" tile. `ProjectsProvider`/`useProjects.js` (localStorage, mirrors `useHistory.js`'s shape).
- ✅ 2026-09-16: each project sorts its own tasks through Today/Next Up/Next Week/Someday — reuses the exact `TaskDndArea`/`TaskDropZone`/`TaskGroup`/`groupKey` machinery the day list and Backlog already use, so drag-and-drop needed zero new logic. New tasks can go straight to Today via a project-only placement option layered on top of `bucketOptions` (kept separate since `ACTIVE` isn't a backlog bucket and Backlog's shift-arrows depend on that array's exact shape).
- ✅ 2026-09-16: tasks carry an optional `project` field (legacy-safe) and show a small muted project-name label on their row when tagged.
