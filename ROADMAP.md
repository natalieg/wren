...
### Phase 2 — Task editing & smarter time input — next up
- Edit an existing task's label and time estimate — flagged as priority now that tasks actually carry a time field worth editing.
...
- Needs real thought on the add/edit UI itself, not just the parsing logic underneath it.
